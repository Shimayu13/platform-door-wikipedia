// app/admin/lines/new/page.tsx - 新規作成
...
import { createLine, type LineInput } from "@/lib/actions"
...
export default function NewLinePage() {
    ...
    const loadCompanies = async () => {
        try {
            ...
            setMessage({ type: "error", text: "鉄道会社データの取得に失敗しました" })
        } finally {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const result = await createLine(lineInput, user.id)

            if (result.success) {
                setMessage({
                    type: "success",
                    text: result.message || "路線を作成しました"
                })
                ...
                setMessage({
                    type: "error",
                    text: result.error || "路線の作成に失敗しました"
                })
            }
        } catch (error) {
        ...
        <LineManagementAccess>
            <div className="min-h-screen bg-gray-50">
                        ...
                        <div className="flex justify-between items-center py-4">
                            <div className="flex items-center space-x-4">
                                ...
                                <div className="border-l pl-4">
                                    ...
                                    <p className="text-sm text-gray-600">新しい路線を追加します</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <main className="max-w-3xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                    ...
                    <Card className="mt-6">
                        <CardHeader>
                            <CardTitle className="text-sm">路線作成について</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="space-y-2 text-sm text-gray-600">
                                <p>• 路線名と鉄道会社は必須項目です</p>
                                <p>• 路線カラーは16進数（例：#009639）で入力してください</p>
                                <p>• 作成後は路線管理画面から編集・削除が可能です</p>
                                <p>• 路線に駅が登録されている場合は削除できません</p>
                            </div>
                        </CardContent>
                    </Card>
                </main>
